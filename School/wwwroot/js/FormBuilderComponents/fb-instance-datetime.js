(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() :
        typeof define === 'function' && define.amd ? define(factory) :
            (global.VueFBInstanceDateTime = factory());
}(this, (function () {
    var index = {
        name: 'fb-instance-datetime',
        ...
        computed: {
            dateValue: function () {
                return this.value.instanceProperties.dateValue;
            },
            dateValueFormatted: function (){
                return moment(this.value.instanceProperties.dateValue).format('YYYY/MM/DD')
            },
            timeValue: function () {
                return this.value.instanceProperties.timeValue;
            },
            timeValueFormatted: function(){
                return moment(this.value.instanceProperties.timeValue).format('hh:mm A')
            },
        ...
        methods: {
            ...
            validate: function () {
                if (this.isMandatory === true && this.visible == true) {
                    if (this.value.definitionProperties.captureDate == true && this.value.definitionProperties.captureTime == false && this.dateValue != null && this.dateValue != '') {
                        this.value.isValid = true;
                    }
                    else if (this.value.definitionProperties.captureDate == false && this.value.definitionProperties.captureTime == true && this.timeValue != null && this.timeValue != '') {
                        this.value.isValid = true;
                    }
                    else if (this.value.definitionProperties.captureDate == true && this.value.definitionProperties.captureTime == true && this.timeValue != null && this.timeValue != ''
        ...
        template: `<div class="number fb-instance-item" v-if="value.childCode == 'DATETIME' && visible == true">
                        <div class="row">
                              <div class="col-md-12">
                              ...
                              <div class="col-md-12">
                                    ...
                                    <div class="tab-content px-1 pt-1 border-left border-bottom border-right">
                                        <div role="tabpanel" class="tab-pane active p-1" v-bind:id="'fbitemresult' + value.formDefinitionID + '_' + sindex + '_' + cindex + '_' + iindex + '_' + iiindex + '_' + diindex" v-bind:aria-labelledby="'fbitemresult-tab' + value.formDefinitionID + '_' + sindex + '_' + cindex + '_' + iindex + '_' + iiindex + '_' + diindex">
                                            <div class="row">
                                                <div class="col-md-6">
                                                     <div class="row mar-btm" v-if="value.definitionProperties.captureDate == true">
                                                        <div class="col-xs-12">
                                                            <div class="form-group has-feedback" style="max-width:250px" v-if="readonly == false">
                                                                <vue-datetime type="date"
                                                                                v-model="value.instanceProperties.dateValue"
                                                                                input-class="form-control"
                                                                                input-icon-class="pli-calendar icon-lg form-control-feedback"
                                                                                format="D"
                                                                                :min-datetime="(value.definitionProperties.allowPastDates) ? null : new Date().toString()"
                                                                                auto></vue-datetime>
                                                            </div>
                                                            <div class="form-group has-feedback" style="max-width:250px" v-if="readonly == true">
                                                                <i class="pli-calendar icon-lg"></i> {{dateValueFormatted}}
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="row mar-btm" v-if="value.definitionProperties.captureTime == true">
                                                        <div class="col-xs-12">
                                                            <div class="form-group has-feedback" style="max-width:250px" v-if="readonly == false">
                                                                <vue-datetime type="time" input-id="timeValue"
                                                                                v-model="value.instanceProperties.timeValue"
                                                                                input-class="form-control"
                                                                                input-icon-class="pli-clock icon-lg form-control-feedback"
                                                                                format="t"
                                                                                title="Time"
                                                                                :min-datetime="(value.definitionProperties.allowPastTimes) ? null : new Date().toString()"
                                                                                use12-hour
                                                                                auto>
                                                                </vue-datetime>
                                                            </div>
                                                            <div class="form-group has-feedback" style="max-width:250px" v-if="readonly == true">
                                                                <i class="pli-clock icon-lg"></i> {{timeValueFormatted}}
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        <div role="tabpanel" class="tab-pane" v-bind:id="'fbitemfeedback' + value.formDefinitionID + '_' + sindex + '_' + cindex + '_' + iindex + '_' + iiindex + '_' + diindex" v-bind:aria-labelledby="'fbitemfeedback-tab' + value.formDefinitionID + '_' + sindex + '_' + cindex + '_' + iindex + '_' + iiindex + '_' + diindex">
                                              <fb-instance-feedback v-model="value.instanceItem" :isfaulureresultchosen="isFaulureResultChosen()" :readonly="readonly"></fb-instance-feedback>
                                        </div>
                                    </div>
                              </div>
                        </div>
