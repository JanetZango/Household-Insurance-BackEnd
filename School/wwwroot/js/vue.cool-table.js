(function (global, factory) {
    ...
    var index = {
        ...
        template: `<div class="table-responsive"><table v-bind:class="tableClass" cellspacing="0" width="100%">
            <thead>
                <tr>
        ...
        <div class="row">
            <div class="col-auto page-number-table">
                <select v-model="setPageSize" v-on:change="onPageSizeChange($event)" style="width: 80px" class="form-control">
                    <option v-for="option in pageSizeList" :value="option">{{option}}</option>
                </select> 
            </div>
            <div class="col-auto mr-auto" style="margin-top:8px">
            per page | Showing results {{pagination.skip + 1}} to {{(pagination.skip + setPageSize) > pagination.totalRecords ? pagination.totalRecords : (pagination.skip + setPageSize) }} of {{pagination.totalRecords}}
            </div>
            <div class="col-auto">
                <ul v-if="pager.pages && pager.pages.length" class="pagination">
                    <li v-bind:class="['page-item',{'disabled': pager.currentPage === 1}]">
                        <button type="button" class="page-link" v-on:click="setPage(1)"><i class="la la-angle-double-left"></i></button>
                    </li>
                    <li v-bind:class="['page-item',{'disabled': pager.currentPage === 1}]">
                        <button type="button" class="page-link" v-on:click="setPage(pager.currentPage - 1)"><i class="la la-angle-left"></i></button>
                    </li>
                    <li v-for="page in pager.pages" v-bind:class="['page-item',{'active': pager.currentPage === page}]">
                        <button type="button" class="page-link" v-on:click="setPage(page)">{{page}}</button>
                    </li>
                    <li v-bind:class="['page-item',{'disabled': pager.currentPage === pager.totalPages}]">
                        <button type="button" class="page-link" v-on:click="setPage(pager.currentPage + 1)"><i class="la la-angle-right"></i></button>
                    </li>
                    <li v-bind:class="['page-item',{'disabled': pager.currentPage === pager.totalPages}]">
                        <button type="button" class="page-link" v-on:click="setPage(pager.totalPages)"><i class="la la-angle-double-right"></i></button>
                    </li>
                </ul>
            </div>
        </div>
